...
ChucK에서 여러번 재사용하고 싶은 코드에 이름을 붙여두고 불러 쓰도록 하는 구조를 <u>함수(function)</u>라고 한다. 이름을 붙이는 구조를 <u>함수 선언(function declaration)</u>이라고 하고, 이름을 부르는 구조를 <u>함수 호출(function call)</u>이라고 한다. 예를 들어, MIDI 음을 담는 변수를 다음과 같이 선언하고 초기값을 지정했다고 하자.
...
함수 선언은 `fun`이라는 키워드로 시작한다. 바로 뒤에 이어서, 함수를 호출하면 리턴해줄(돌려줄) 값의 타입을 기술한다. 위 함수의 경우 `int` 타입의 값을 리턴하도록 하였다. 그리고 이어서 함수의 이름을 기술한다. 위 함수의 이름은 `changeOctave`라고 지었다. 이어서 괄호 안에 쉼표로 구분하여 나열한 것들을 <u>형식 파라미터(formal parameter)</u> 또는 그냥 <u>파라미터(parameter)</u> 라고 한다. 파라미터의 개수에는 제한이 없다. 각 파라미터는 변수 선언과 마찬가지로 타입과 변수를 빈칸을 사이에 두고 적는다. 위 함수의 경우, 두 파라미터 모두 `int` 타입이며 변수 이름은 각각 `note`와 `step` 이다. `note`는 MIDI 음을 나타내도록, `step`은 조정할 옥타브 단위를 나타내도록, 이름을 지었다. 파라미터가 필요없는 경우, `()`와 같이 빈 괄호만 둔다. 중괄호(`{}`)로 둘러싸인 부분에는 코드가 들어가는데 함수의 <u>몸체(body)</u>라고 한다. 위 함수의 경우 코드가 단 한 문장인데, 리턴문이다. 리턴문은 `return` 이라는 키워드로 시작하며, 이어서 식을 기술한다. `step` 변수 값이 양수이면 그만큼 옥타브를 올리고, 음수이면 그만큼 옥타브를 내리도록 식을 기술하였다. 함수를 호출하면 이 식을 계산한 결과 값을 리턴한다.

함수 호출은 호출할 함수 이름을 쓰고, 이어서 괄호 안에 쉼표로 구분하여 식을 나열한다. 이를 <u>실제 파라미터(actual parameter)</u> 또는 <u>인수(argument)</u> 라고 한다. 인수는 반드시 해당 함수의 형식 파라미터와 개수와 타입이 일치해야 한다. 함수를 호출하면 이 인수들을 차례로 모두 계산하고, 그 결과값을 해당 함수의 형식 파라미터 변수에 차례로 전달하여 지정한다. 그리고 나서 함수의 몸체를 실행한다. 몸체 실행 중에 리턴문을 만나면 그 식의 계산 결과값을 리턴한다. 위 함수 호출을 실행하면, 인수인 `mynote`의 값과 이어서 언급한 정수 값이 각각 파라미터 변수 `note`와 `step`에 지정되고, 이어서 `note + (step * 12)` 식을 계산한 결과값을 리턴해준다.
...
이런 패턴의 코드블록을 여러 번 쓰므로 함수로 만들어 쓰면 좋다. 이 코드블록 패턴에서 어떤 부분이 변하는지 찾아보자. 바로 눈의 띄는 것은 소리 내기 전에 세팅하는 주파수와 소리크기의 값이다. 이 둘을 파라미터로 정하고 기존 코드 하단에 추가로 다음과 같이 함수를 선언할 수 있다.
...
여기서 `Osc`는 여러 진동기 `SinOsc`, `SqrOsc`, `TriOsc`, `SawOsc`를 모두 대표하는 타입의 이름이다. 따라서 어떤 진동기를 인수로 전달해도 문제없이 작동한다. 이전 코드에서 `off` 이제 이 프로시저 함수는 진동기, 주파수, 소리크기, 시간이 주어지면 소리를 내준다. 그리고 이 전 코드에서 무음 소리 크기 값인 `0.0`은 여러번 반복 사용될 뿐 아니라, 다른 값으로 바꾸고 싶은 경우 한번만 수정하면 되도록 하기 위해서 `off` 변수를 선언하여 사용하였다. 그런데 이 부분이 함수 안으로 들어가면서 한번만 쓰면 되므로 변수를 사용할 필요가 없어졌다. 따라서 `off`는 `0.0`으로 바꾸었다. 이 값을 수정하고 싶으면, 함수에서 이 값을 바꾸기만 하면된다. 이 프로시저를 활용하도록 위 코드를 다시 작성하면 다음과 같다.
...
함수를 잘 활용하여 함수를 재작성해보니 코드가 훨씬 깔끔하고 가독성이 훨씬 좋아졌다. 

이제 앞 장에서 공부한 프로그램을 `makeSound` 함수를 활용하여 재작성하면 아래와 같다. 아래 코드를 이전 코드를 비교하여 어떤 점이 좋아졌는지 효과를 스스로 판단해보자.
...
앞에서 공부한 `changeOctave` 함수는 둘째 인수 `step`으로 몇 옥타브를 올릴지 내릴지 정하게 하였다. 한 옥타브만 올리는 경우가 아주 잦다고 가정하면, `step`의 기본값을 `1`로 정하고 이 인수를 명시하지 않아도 되게 하면 편리하다. 다시 말해 `step`이 1이 아닌 경우는 `changeOctave(60,-1)`과 같이 호출하고, `step`이 1인 경우에 한해서 `changeOctave(60)`과 같이 둘째 인수를 생략하고 호출해도 한 옥타브만 올려서 `72`를 리턴하게 해주면 편하겠다는 뜻이다. 그러기 위해서는 같은 이름의 함수를 다음과 같이 추가로 작성하면 된다.
...
그런데 같은 이름의 함수를 두 개 작성해도 괜찮을까? 다행히도 이런 경우를 고려하여 ChucK 프로그래밍 언어는 <u>함수 이름의 중복(overloading)</u>을 허용한다. (각주: 사실 대부분의 프로그래밍 언어가 이러한 유형의 함수 이름 중복을 허용하고 있다.) 같은 이름을 가진 함수의 중복 사용을 가능하게 하려면 함수의 파라미터의 개수를 다르게 하거나, 개수가 같더라도 파라미터의 타입이 최소한 하나 다르게 하면 된다. 그러면 프로그래밍 시스템이 파라미터의 개수와 타입을 검사하여 같은 이름의 함수 중에서 가장 적합한 함수를 찾아서 호출 해준다. 따라서 `changeOctave` 함수를 두 개 모두 정의 해놓고 필요에 따라 둘 중 하나를 적절히 호출해 쓰도록 해도 전혀 문제가 없다.
...
#### 사례 학습 : `makeSound` 함수의 유연성 증대

음에서 무음으로 처리하는 시간을 사용자가 조절할 수 있도록, 다음과 같이 파라미터를 추가하여 `makeSound` 함수 활용의 유연성을 늘릴 수도 있다.
...
그러면 다음 프로그램과 같이, 각 진동기의 주파수와 소리 크기를 이 함수를 호출하여 설정한 다음, 동시에 소리나도록 시간을 보내면 된다.
...
지난 실습과 숙제로 작성한 다음 두 프로그램을 오늘 배운 함수 `makeSound`를 활용하여 재작성하자.
